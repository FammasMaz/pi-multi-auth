...
test("plan filter footer action surfaces when supported", () => {
	const actions = resolveFooterActions({
		focusedPane: "accounts",
		renameMode: false,
		hasProviderSelection: true,
		...
		hasBatchSelection: false,
		selectedAccountMarked: false,
		supportsPlanFilter: true,
		planFilterLabel: "Plus/Paid",
	});

	assert.ok(actions.includes("[Tab] Plan: Plus/Paid"));
});

test("plan filter footer action is hidden when unsupported", () => {
	const actions = resolveFooterActions({
		focusedPane: "accounts",
		renameMode: false,
		hasProviderSelection: true,
		...
		hasBatchSelection: false,
		selectedAccountMarked: false,
		supportsPlanFilter: false,
	});

	assert.ok(!actions.some((action) => action.startsWith("[Tab] Plan")));
});
...
test("plan label resolution prefers live usage snapshot then token tier", () => {
	const withSnapshot = makeCredential("a", {
		planTier: "free",
		usageSnapshot: {
			timestamp: 0,
			provider: "openai-codex",
	...
	assert.equal(resolveCredentialPlanLabel(withSnapshot), "ChatGPT Plus");

	const tierOnly = makeCredential("b", { planTier: "plus" });
	assert.equal(resolveCredentialPlanLabel(tierOnly), "plus");

	assert.equal(resolveCredentialPlanLabel(makeCredential("c")), null);
});

test("plan tier classification splits paid from free and leaves unknown null", () => {
	assert.equal(classifyCredentialPlanTier(makeCredential("a", { planTier: "plus" })), "paid");
	assert.equal(classifyCredentialPlanTier(makeCredential("b", { planTier: "team" })), "paid");
	assert.equal(classifyCredentialPlanTier(makeCredential("c", { planTier: "free" })), "free");
	assert.equal(classifyCredentialPlanTier(makeCredential("d")), null);
});

test("plan filter keeps unknown-plan accounts visible across all modes", () => {
	const credentials = [
		makeCredential("plus", { planTier: "plus" }),
		makeCredential("free", { planTier: "free" }),
		makeCredential("unknown"),
	];

	assert.deepEqual(
		filterCredentialsByPlan(credentials, "all").map((c) => c.credentialId),
		["plus", "free", "unknown"],
	);
	assert.deepEqual(
		filterCredentialsByPlan(credentials, "paid").map((c) => c.credentialId),
		["plus", "unknown"],
	);
	assert.deepEqual(
		filterCredentialsByPlan(credentials, "free").map((c) => c.credentialId),
		["free", "unknown"],
	);
});
